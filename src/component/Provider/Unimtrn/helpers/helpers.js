...
export const fixNameUnimtrn = (el) => {
  ...
  const fixZFold7 = fixCPH2581.replace("Z Fold7", "Z Fold 7");
  const fixZFlip7 = fixZFold7.replace("Z Flip7", "Z Flip 7");
  const fixJade = fixZFlip7.replace("Jade ", "");
  ...
  const fixBuds2Pro = replaceFire.replace("Buds2 PRO", "Buds 2 PRO");
  ...
  const replaceAW = replacecheck.replace("AW 1", "S1");
  const replaceS1 = replaceAW.replace("Series 1", "S1");
